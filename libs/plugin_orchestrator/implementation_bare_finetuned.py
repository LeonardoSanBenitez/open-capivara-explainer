...
from libs.utils.connector_llm import ConnectorLLM, ChatCompletionMessage, ChatCompletionMessageResponse, OpenaiFunctionCall, DefinitionOpenaiTool
...
class OrchestratorBareFinetuned(OrchestratorBare):
    ...
    Ideally (but in practice don't seem to be true), we do not explain how to call a function nor which functions are available, but we still have to custom-detect the function calls.
    ...
    '''
    inform_last_step: bool = False
    inform_step: bool = False
    max_steps_exceeded_behavior: Literal['error', 'return_last'] = 'return_last'
